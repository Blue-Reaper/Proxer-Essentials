...
function addPefMenu() {
    $('#leftNav').append(
        $(
            '<li class="topmenu"><a href="javascript:;">Essentials ▾</a><ul id="pef_menu"></ul></li>'
        )
    );
    // Erzeugt 'Einstellungen' in Essentials
    $('#pef_menu').append(
        $('<li><a href="/pef?s=settings#top">Einstellungen</a></li>')
    );
}
...
function createPefSettings() {
    if (window.location.pathname.split('/')[1] === 'pef') {
        ...
        $('#simple-navi').append(
            $(
                '<li id ="pef_Settings"><a data-ajax="true" href="/pef?s=settings#top">Einstellungen</a></li>'
            )
        );
        ...
        if (location.search === '' || location.search === '?s=settings') {
            ...
            inhalt.append(
                $('<div class="floatLeft version">' + GM_info.script.version + '</div>')
            );
...
function showModules(pef_module: any) {
    ...
    for (let singleModule of pefModulList) {
        ...
        moduleBox.append(
            $('<div class="autor">by ' + singleModule.autor + '</div>')
        );
        // IDEA: Button für Details hinzufügen
        let modulStatus = $(
            '<i id="' + singleModule.id + '_StatusImg" class="fa fa-2x pointer"/>'
        );
